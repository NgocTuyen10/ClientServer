...
namespace QuanLySieuThi
{
    public partial class KhachHang : Form
    {
        ...
        private void addButton_Click(object sender, EventArgs e)
        {
            ...
            //string checkQuery = string.Format(@"SELECT dbo.UF_ma('{0}') FROM dbo.QuayHang", maquay);
            //string query = string.Format(@"EXECUTE dbo.USP_InsertQH @maquay = '{0}', @tenquay = N'{1}'", maquay, tenquay);
            


            if (maKHTextBox.Text.Trim().Length != 0 || maKHTextBox.Text.Trim().Equals("KHxxx"))
            {
                string query = string.Format(@"EXEC dbo.insertKH @makh = '{0}',@tenKH = N'{1}',@sdt = '{2}',@diachi = N'{3}'", maKH, tenKH, sdt, diaChi);    
        ...
        private void changeButton_Click(object sender, EventArgs e)
        {
            ...
            if (maKHTextBox.Text.Trim().Length != 0 || maKHTextBox.Text.Trim().Equals("KHxxx"))
            {
                string query = string.Format(@"EXEC dbo.updateKH @makh = '{0}',@tenKH = N'{1}',@sdt = '{2}',@diachi = N'{3}'", maKH, tenKH, sdt, diaChi);    
                MessageBox.Show("" + myControl.ExecuteMyQuery(query));                 
                showData();
            }
            else
